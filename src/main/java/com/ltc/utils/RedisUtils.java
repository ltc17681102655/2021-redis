...
public class RedisUtils {
    ...
    public <V> List<V> batchGet(String keyPrefix, List<String> ids, Class<V> clazz) {
        return batchGet(keyPrefix, ids, clazz, null);
    }

    public Map<String, String> batchGet(String keyPrefix, List<String> ids) {
        ...
        try {
            list = redisTemplate.executePipelined(new RedisCallback<String>() {
                ...
                public String doInRedis(RedisConnection connection) throws DataAccessException {
                    ...
                    for (String id : ids) {
                        redisConnection.get(keyPrefix + id);
                    }
                    return null;
                }
            });
        } catch (Exception e) {
            log.warn("[batchGet] redis operations failed, keys:" + keyPrefix + ", ids:" + JSON.toJSONString(ids), e);
        }
        ...
        if (!CollectionUtils.isEmpty(list)) {
            for (int i = 0; i < list.size(); i++) {
                ...
                if (!StringUtils.isEmpty(value)) {
                    resultMap.put(ids.get(i), value.toString());
                }
            }
        }
        return resultMap;
    ...
    public <V, T extends List<String>, R extends List<V>> List<V> batchGet(String keyPrefix, List<String> ids, Class<V> clazz, Function<T, R> function) {
        ...
        if (CollectionUtils.isEmpty(list)) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (!StringUtils.isEmpty(obj)) {
                    resultList.add(JSON.parseObject(obj.toString(), clazz));
                } else {
                    missingList.add(ids.get(i));
                }
            }
        }
        ...
        if (!CollectionUtils.isEmpty(missingList) && function != null) {
            ...
            R dbList = functionByCounter(keyPrefix + idList.hashCode(), function, idList, "missing ids");
            if (!CollectionUtils.isEmpty(dbList)) {
                resultList.addAll(dbList);

                for (int i = 0; i < dbList.size(); i++) {
                    set(keyPrefix + idList.get(i), JSON.toJSONString(dbList.get(i)));
                }
            }
        }
        return resultList;
    ...
    public <R extends Map<Long, T>, T> List<T> zrange(String key, long start, long end, Class<T> clazz, long expire, Supplier<R> supplier) {
        try {
            Set<String> set = zSetOperations.range(key, start, end);
            if (!CollectionUtils.isEmpty(set)) {
                return JSON.parseArray(Lists.newArrayList(set).toString(), clazz);
            }
            throw new RuntimeException("redis value is empty");
        } catch (Exception e) {
            R map = supplierByCounter(key, supplier, "[zset range] " + e.getMessage());
            if (!CollectionUtils.isEmpty(map)) {
                List<T> list = Lists.newArrayList();
                map.forEach((k, v) -> {
                    zadd(key, Double.parseDouble(k.toString()), JSON.toJSONString(v));
                    list.add(v);
                });
                redisTemplate.expire(key, expire, TimeUnit.SECONDS);
                return list;
    ...
    public <V, T extends List<String>, R extends List<V>> List<V> batchHGet(String key, List<String> fields, Class<V> clazz, Function<T, R> function) {
        ...
        if (!CollectionUtils.isEmpty(missingList) && function != null) {
            ...
            if (!CollectionUtils.isEmpty(dbList)) {
                ...
                for (int i = 0; i < dbList.size(); i++) {
                    hset(key, idList.get(i), JSON.toJSONString(dbList.get(i)));
                }
            }
        }
